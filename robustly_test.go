...
func TestRobustly1(t *testing.T) {
	...
	panics := Run(func() { panicRateIters(time.Second, 5, &tries) })
	if panics != 5 {
...
func TestRobustly3(t *testing.T) {
	...
	tries := 0
	Run(func() { panicRateIters(time.Millisecond*300, 500, &tries) })
...
func TestRobustly4(t *testing.T) {
	...
	tries := 0
	panics := Run(func() { panicRateIters(time.Millisecond*300, 2, &tries) })
	if panics != 2 {
